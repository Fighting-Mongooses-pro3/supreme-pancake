...
export default function buildTest() {
  ...
  async function addCard(loc) {
    ...
    let cardEls = cardInfo.map((e, i) => {
      ...
      return (
        <Draggable onStop={handleStop} axis="y" grid={[20, 20]} scale={1}>
          <div
            style={{ top: e.previousY + e.y + "%" }}
            id={`card${i}`}
            className={` absolute  ${side} w-2/5 h-1/4 border bg-red-400  `}
          >
            {e.text}
          </div>
        </Draggable>
        ...
        <div className="flex justify-center items-center h-screen">
          <div>
            <div className="flex justify-around">
              ...
              <br />
              <button className="border" onClick={() => addCard("l")}>
                add left{" "}
              </button>
